...
def fit_3dmm_for_a_video(
    ...
    for i in range(int((num_frames-1)/batch_size+1)):
        ...
        if id_mode != 'global':
            id_para[sel_ids].data = sel_id_para.data.detach().clone()
        exp_para[sel_ids].data = sel_exp_para.data.detach().clone()
        euler_angle[sel_ids].data = sel_euler_angle.data.detach().clone()
        trans[sel_ids].data = sel_trans.data.detach().clone()
