    ...
    async def streaming_transcribe(self, filename, config=None):
        ...
        async def transcriber(websocket):
            ...
            async for msg in websocket:
                msg = json.loads(msg)
                if msg["final"]:
                    transcribed_text += msg["alternatives"][0]["text"] + " "
...
def transcribe_speech_rtzr(filename, client_id, client_secret):    
    client = RTZROpenAPIClient(client_id, client_secret)
    loop = asyncio.get_event_loop()
    transcription = loop.run_until_complete(client.streaming_transcribe(filename))
...
def main():
    ...
    while True:
        ...
        user_input = transcribe_speech_rtzr("user_input.wav", client_id, client_secret)
        ...
        csv_file_path = './records/records_returnzero_pydub.csv'
        df.to_csv(csv_file_path, index=False)
        latency_csv_file_path = './records/latency_records_returnzero.csv'
